...
function GameDetails(servername, serverurl, mapname, maxplayers, steamid, gamemode) {
	document.getElementById("bio").textContent = servername;
	document.getElementById("map").textContent = mapname;
	document.getElementById("steamid").textContent = SteamID;
	//document.getElementById("bio").textContent = maxplayers;
	SteamID = convertSteamID64toSteamID(new BigNumer(steamid));
}

function convertSteamID64toSteamID(sID64){
	steamID64 = new BigNumber(sID64);
	zwoa = new BigNumber("2");
	SteamWuascht = new BigNumber("76561197960265728");
	console.log(steamID64);
	return "STEAM_0:" + steamID64.mod(zwoa) + ":" + (steamID64.subtract(steamID64.mod(zwoa)).subtract(SteamWuascht)).divide(zwoa);
}
